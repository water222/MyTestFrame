...
import com.vimalselvam.testng.listener.ExtentTestNgFormatter; //使用的应该不是ExtentReport原来的监听器
import org.testng.*;
...
public class MyExtentTestNgFormatter extends ExtentTestNgFormatter {
    ...
    public MyExtentTestNgFormatter() {
        ...
        String reportPathStr = System.getProperty("reportPath");//可以传递这个参数吗？验证这里输出的是null
        File reportPath;
        ...
        // 首先找到具体要发送的报告“report.html”和"emailable-report.html"，我想知道他是在哪段代码中生成了这两个文件，应该是执行用例中吧？
...
//        如果cdn.rawgit.com访问不了，可以设置为：ResourceCDN.EXTENTREPORTS或者ResourceCDN.GITHUB
    ...
    public void onStart(ISuite iSuite) {
        if (iSuite.getXmlSuite().getTests().size() > 0) {
            //createTest创建测试，返回的是一个 ExtentTest对象
